...
CREATE TABLE IF NOT EXISTS UserTeam(
	TeamID INT(4) UNSIGNED PRIMARY KEY,
    TeamName VARCHAR(40) NOT NULL);
CREATE TABLE IF NOT EXISTS UserGroup(
	GroupID INT(4) UNSIGNED PRIMARY KEY,
    GroupName VARCHAR(40) NOT NULL);
CREATE TABLE IF NOT EXISTS UserRole(
	RoleID INT(4) UNSIGNED PRIMARY KEY,
    RoleName VARCHAR(40) NOT NULL);
CREATE TABLE IF NOT EXISTS Multipliers(
	MulID BIGINT(12) UNSIGNED PRIMARY KEY,
    MulValue INT(3) NOT NULL);
CREATE TABLE IF NOT EXISTS Gender(
	GenderID INT(1) PRIMARY KEY,
    GenderName VARCHAR(6) NOT NULL);
CREATE TABLE IF NOT EXISTS Category(
	CategoryID INT(4) PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS Users(
    ...
    FOREIGN KEY (UserTeam) REFERENCES UserTeam(TeamID),
    FOREIGN KEY (UserGroup) REFERENCES UserGroup(GroupID),
    FOREIGN KEY (UserRole) REFERENCES UserRole(RoleID));
